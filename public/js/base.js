(()=>{
  ...
  function activeChapterIndex(){const v=qs("#rpm-active-chapter")?.value||qs("#rpm-toc-section")?.dataset?.activeChapter||"0";const n=parseInt(v,10);return Number.isFinite(n)&&n>=0?n:0}
  ...
  function applyActiveChapter(){const section=qs("#rpm-toc-section");if(!section)return;const idx=parseInt(section.dataset.activeChapter||activeChapterIndex()||0,10)||0;const tabs=qsa(".rpm-chapter-tabs>li");const panes=qsa(".rpm-chapter-switcher>li");if(!tabs.length||!panes.length)return;tabs.forEach((li,i)=>li.classList.toggle("uk-active",i===idx));panes.forEach((li,i)=>li.classList.toggle("uk-active",i===idx));setActiveChapterIndex(idx)}
  ...
  document.addEventListener("DOMContentLoaded",()=>{bindProgressForm();bindChapterTabs();bindConfirmForms();bindImagePreview();applyActiveChapter();syncMindmapDetails();initCharts();initMindmap();window.addEventListener("resize",resizeCharts,{passive:true})});
  ...
  document.body.addEventListener("htmx:afterSettle",()=>{bindProgressForm();bindChapterTabs();bindConfirmForms();bindImagePreview();applyActiveChapter();syncMindmapDetails();initCharts();initMindmap();resizeCharts()})
})();
