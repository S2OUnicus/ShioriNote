...
function inst_install(bool $devMode): array
{
    ...
    if ($loginId === '' || !preg_match('/^[A-Za-z0-9]{1,16}$/', $loginId)) { throw new RuntimeException('管理者IDの形式が正しくありません。'); }
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'account_save') {
            ...
            if (!preg_match('/^[A-Za-z0-9]{1,16}$/', (string)inst_post_value('login_id',''))) { throw new RuntimeException('IDは16桁以内の半角英数字で指定してください。'); }
